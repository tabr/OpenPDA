...
//  Timer.addTask(Buttons.TaskTryUnlockButton);            //13
  ...
  TimerMS.RegisterListener(&TimeUpdate);
  TimerMS.RegisterListener(&Grenade);
...
//PDAMenu.selectMenuID(250);
  ...
  #ifdef DEBUG_USART
    sprintf (USARTBuffer,"Start %03u:%03u:%03u %01u %01u %01u\r\n", TimeRTC.hour, TimeRTC.minute, TimeRTC.second, LCD_DMA_Activated, BackLight.isBacklightONInverted, (LCD_DMA_Activated & BackLight.isBacklightONInverted));
    sendString(USARTBuffer);
  #endif
  StalkerUI.ClearDisplay();
  StalkerUI.DisplayString(1, StalkerUI_Class::UI_LCD_LINE_0, STR_TIME_SYNC_AWAITING);
  StalkerUI.RequestUpdate();
  while(timeSyncReceivedAtLastOnce == false)
    {
    StalkerLayer2->L1.isEnvironmentActive();//wait for time sync
    StalkerLayer2->L1.L0.packetProcessed();
    if (PowerManager.isDischarged())
      {
