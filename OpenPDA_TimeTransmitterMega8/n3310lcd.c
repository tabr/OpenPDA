...
bool power_down  = false;            //power-down control: 0 - chip is active, 1 - chip is in PD-mode
...
void LcdChr (int chr, uint8_t x, uint8_t y);
...
//ASCII
const char table1[0x0500] PROGMEM =
//char table[0x0500] =
{
...
//        LCD_CPORT.LCD_CPIN = 0;                //Enable display controller (active low)
        //cbi(LCD_CPORT,LCD_CPIN);                //Enable display controller (active low)
		LcdPinSCE::setOutLow();
        if (cmd)
			{
...
//        LCD_CPORT.LCD_CPIN = 1;                //Disable display controller
        //sbi(LCD_CPORT,LCD_CPIN);                //Disable display controller
...
void LcdClear (void)    //Clears the display
    ...
    //for (i = 0; i < LCD_CACHSIZE; i++) LcdCache[i] = 0;    //забиваем всю память 0
		memset(LcdCache, 0, LCD_CACHSIZE);
		LcdUpdate ();
        }
...
void LcdChr (char* chr, uint8_t x, uint8_t y)
	{
	LcdGotoXYFont (x, y);
	LcdChr (chr[0]);
	}
void LcdChr (int ch)    //Displays a character at current cursor location and increment cursor location
     {
        ...
        for ( i = 0; i < 5; i++ ) LcdCache[LcdCacheIdx++] = pgm_read_byte(&table1[(ch*5+i)]);    //выделяем байт-столбик из символа и грузим в массив - 5 раз
...
void LcdChrBold (int ch)    //Displays a bold character at current cursor location and increment cursor location
     {
         ...
         for ( i = 0; i < 5; i++ )
                 {
                 c = pgm_read_byte(&table1[(ch*5+i)]);        //выделяем столбец из символа
...
#define LCD_MENU_BAR_LINES_BOTTOM		(48-0)//34
