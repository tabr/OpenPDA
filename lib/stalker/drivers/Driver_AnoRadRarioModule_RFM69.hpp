...
//		const static uint8_t RFM69_REGISTER_37_PACKETCONFIG1_3_CRC_AUTO_CREAR_OFF									= 3;
		...
		void setTXBuffer(uint8_t* buf);						//нужно вызывать перед инициализцаией передатчика
		void setRXBuffer(uint8_t* buf);						//нужно вызывать перед инициализцаией приёмника
		void setPacketSize(uint8_t size);					//нужно вызывать перед инициализцаией
		void writeRegister(uint8_t addr, uint8_t value);
		uint8_t readRegister(uint8_t addr);
		void TXMode(void);									//перевести в режим приёмника
		void getPacket(void);
