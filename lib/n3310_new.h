...
/*const static LCD_LINE artLines[]=
	{
	LCD_LINE_2,
	LCD_LINE_3,
	LCD_LINE_4,
...
#define LCD_CHAR_WIDTH		14
//#define USING_WATERMARKS	1


//char lcd_buf[15];        //текстовый буфер для вывода на LCD
//char lcd_buf[32];        //текстовый буфер для вывода на LCD moved to LcdClass
...
void LcdString (LCD_LINE y)                //Displays a string at current cursor location
  {
  LcdString(1, y);
  }
...
#define LCD_CLK_PORT1						PORTE_OUT    // тактирование SPI, должен быть соответствующий пин аппаратного SPI, если он используется
#define LCD_CLK_DDR1							PORTE_DIR
#define LCD_CLK_PIN1							PIN7_bp

//miso 6

#define LCD_MOSI_PORT1						PORTE_OUT    // выход данных SPI, должен быть соответствующий пин аппаратного SPI, если он используется
#define LCD_MOSI_DDR1						PORTE_DIR
#define LCD_MOSI_PIN1						PIN5_bp

#define LCD_DC_PORT1							PORTE_OUT    // выбор команда-данные; любой пин любого порта
#define LCD_DC_DDR1							PORTE_DIR
#define LCD_DC_PIN1							PIN4_bp

#define LCD_CE_PORT1							PORTE_OUT    // Chip-Select; любой пин любого порта
#define LCD_CE_DDR1							PORTE_DIR
#define LCD_CE_PIN1							PIN3_bp

#define LCD_SS_PORT1							PORTE_OUT    // ChipSelect SPI, должен быть соответствующий пин аппаратного SPI и настроен на выход
#define LCD_SS_DDR1							PORTE_DIR
#define LCD_SS_PIN1							PIN3_bp

#define LCD_RST_PORT1						PORTE_OUT    // сброс экрана; любой пин любого порта
#define LCD_RST_DDR1							PORTE_DIR
#define LCD_RST_PIN1							PIN2_bp
...
//void LcdUpdateDoNotUse     ( void );   // Копирование буфера в ОЗУ дисплея
void LcdImage      ( const uint8_t *imageData );   // Рисование картинки из массива в Flash ROM
//void LcdContrast   ( uint8_t contrast );   // Установка контрастности дисплея
uint8_t LcdGotoXYFont ( uint8_t x, uint8_t y );   // Установка курсора в позицию x,y
uint8_t LcdChr        ( LcdFontSize size, uint8_t ch );   // Вывод символа в текущей позиции
uint8_t LcdStr        ( LcdFontSize size, uint8_t dataArray[] );   // Вывод строки сохраненной в RAM
uint8_t LcdFStr       ( LcdFontSize size, const uint8_t *dataPtr );   // Вывод строки сохраненной в Flash ROM
uint8_t LcdPixel      ( uint8_t x, uint8_t y, LcdPixelMode mode );   // Точка
uint8_t LcdLine       ( uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, LcdPixelMode mode );   // Линия
uint8_t LcdCircle     ( uint8_t x, uint8_t y, uint8_t radius, LcdPixelMode mode);   // Окружность
uint8_t LcdRect       ( uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, LcdPixelMode mode );   // Прямоугольник
uint8_t LcdSingleBar  ( uint8_t baseX, uint8_t baseY, uint8_t height, uint8_t width, LcdPixelMode mode );   // Один 
uint8_t LcdBars       ( uint8_t data[], uint8_t numbBars, uint8_t width, uint8_t multiplier );   // Несколько


static uint8_t  LcdCache [ LCD_CACHE_SIZE ];
// Указатель для работы с LcdCache[]
static int   LcdCacheIdx;
bool LcdCacheChanged = false;
