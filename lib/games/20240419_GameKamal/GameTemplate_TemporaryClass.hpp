...
//		const static uint8_t ITEMID_DETAILS					= 9;
		...
		const static uint16_t ANOMALY_DAMAGE_THRESHOLD			= 128;
		const static uint8_t ANOMALY_DAMAGE_MULTIPLIER			= 2;
		const static uint8_t GRENADE_DAMAGE_MULTIPLIER			= 1;

    const static uint8_t PLAYEREFFECT_ITEM_PLYSBLOCK        = 1;
    const static uint8_t PLAYEREFFECT_ITEM_RADIOPROTECTOR   = 2;
    const static uint8_t PLAYEREFFECT_ITEM_FIRSTAIDKIT      = 3;
    const static uint8_t PLAYEREFFECT_ITEM_BANDAGE          = 4;
    const static uint8_t PLAYEREFFECT_ITEM_ANTIRADIN        = 5;
    const static uint8_t PLAYEREFFECT_SAFE_SHELTER          = 6;
    const static uint8_t PLAYEREFFECT_ART_MEDUSA            = 7;
    const static uint8_t PLAYEREFFECT_ART_CRYSTAL           = 8;
    const static uint8_t PLAYEREFFECT_ART_DUSHA             = 9;
    const static uint8_t PLAYEREFFECT_ART_KAM_CVET          = 10;
    const static uint8_t PLAYEREFFECT_ART_KAM_CVET1         = 11;
    const static uint8_t PLAYEREFFECT_ART_KAM_CVET2         = 12;
    const static uint8_t PLAYEREFFECT_ART_EZH               = 13;
    const static uint8_t PLAYEREFFECT_ART_EZH1              = 14;
    const static uint8_t PLAYEREFFECT_ART_EZH2              = 15;
    const static uint8_t PLAYEREFFECT_ITEM_VODKA1           = 16;
    const static uint8_t PLAYEREFFECT_ITEM_VODKA2           = 17;
    const static uint8_t PLAYEREFFECT_ITEM_VODKA3           = 18;
		/*enum AGRESSOR_TYPE
			{
		...
		enum PLAYER_EFFECTS_NOACTION_VARS
			{
			PLAYER_EFFECTS_NOACTION_VAR_NONE,
			PLAYER_EFFECTS_NOACTION_VAR_CUSTOM
			};
		enum SUIT_ID
			{
			SUIT_ID_NONE,
		...
		enum DamageSource
			...
			DAMAGE_SOURCE_ARTEFACT,
			DAMAGE_SOURCE_GRENADE,
			DAMAGE_SOURCE_DETECTOR_TMP,
			DAMAGE_SOURCE_NUM,
			};
/*
		enum FACTIONS
...
*/
		...
		const static uint8_t QID_STARTQUEST					= 1;
...
//		const static uint8_t QUESTS_NUM						= 3;
		const static uint8_t QUESTS_NUM						= 1;//почемубля!
		...
		const static uint8_t DAMAGE_VALUE_NEUTRON_FLUX		= 2;
